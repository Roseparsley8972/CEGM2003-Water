...
import joblib
    ...
    #max_features: Determines how many features will be selected to use at each branching. So at every branch it does not look at all data, only on a subset of it and use that to draw a conclusion?
def run_rf_model(t_size=0.3, trees=50, max_splits=40, max_features='log2', min_samples_leaf=8, min_samples_split=8, k_num=10, y_var='Recharge RC 50% mm/y', y_predict='R50', aus_file='Australia_grid_0p05_data.csv', seed=42, test_data=False):
    ...
    # Using the non split data, we have already split the data earlier so we don't need it for our new data.
